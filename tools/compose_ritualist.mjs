...
const BORDER   = path.join(__dirname, "../frontend/public/assets/Ritualist border.png");
const REFS_DIR = path.join(__dirname, "output/references/Rare");
const OUT_DIR  = path.join(__dirname, "output/cards/Epic");
...
const ritualists = MEMBERS.filter(m => m.role === "Ritualists");

fs.mkdirSync(OUT_DIR, { recursive: true });
console.log(`Compositing ${ritualists.length} Ritualist members...`);

let ok = 0, skip = 0;

for (const member of ritualists) {
  const id = member.id;
...
console.log(`\nDone: ${ok} cards saved to output/cards/Epic/  |  ${skip} skipped`);

// Auto-copy to frontend/public
const PUB_DIR = path.join(__dirname, "../frontend/public/cards/Epic");
fs.mkdirSync(PUB_DIR, { recursive: true });
for (const file of fs.readdirSync(OUT_DIR)) {
  fs.copyFileSync(path.join(OUT_DIR, file), path.join(PUB_DIR, file));
}
console.log(`Copied to frontend/public/cards/Epic/`);
