...
const BORDER   = path.join(__dirname, "../frontend/public/assets/Radiant or zealot border.png");
const REFS_DIR = path.join(__dirname, "output/references/Epic");
const OUT_DIR  = path.join(__dirname, "output/cards/Legendary");
const MEMBERS  = JSON.parse(fs.readFileSync(path.join(__dirname, "output/members.json"), "utf8"));

const SLOT      = { x: 681, y: 473, w: 1599, h: 1612 };
const NAME_X    = 1200;
const NAME_Y    = 2850;
const FONT_SIZE = 110;
...
console.log(`\nDone: ${ok} cards saved to output/cards/Legendary/  |  ${skip} skipped`);

// Auto-copy to frontend/public
const PUB_DIR = path.join(__dirname, "../frontend/public/cards/Legendary");
fs.mkdirSync(PUB_DIR, { recursive: true });
for (const file of fs.readdirSync(OUT_DIR)) {
  fs.copyFileSync(path.join(OUT_DIR, file), path.join(PUB_DIR, file));
}
console.log(`Copied to frontend/public/cards/Legendary/`);
