...
const BORDER   = path.join(__dirname, "../frontend/public/assets/New team mods border.png");
const REFS_DIR = path.join(__dirname, "output/references/Legendary");
const OUT_DIR  = path.join(__dirname, "output/cards/UltraMax");
const MEMBERS  = JSON.parse(fs.readFileSync(path.join(__dirname, "output/members.json"), "utf8"));

const SLOT      = { x: 681, y: 473, w: 1603, h: 1487 };
const NAME_X    = 1200;
const NAME_Y    = 2650;
const FONT_SIZE = 110;

const { width: W, height: H } = await sharp(BORDER).metadata();
const team = MEMBERS.filter(m => m.role === "Mods" || m.role === "Foundation Team" || m.role === "Foundation Team and mods");

fs.mkdirSync(OUT_DIR, { recursive: true });
console.log(`Compositing ${team.length} Team/Mods members...`);
...
console.log(`\nDone: ${ok} cards saved to output/cards/UltraMax/  |  ${skip} skipped`);

const PUB_DIR = path.join(__dirname, "../frontend/public/cards/UltraMax");
fs.mkdirSync(PUB_DIR, { recursive: true });
for (const file of fs.readdirSync(OUT_DIR)) {
  fs.copyFileSync(path.join(OUT_DIR, file), path.join(PUB_DIR, file));
}
console.log(`Copied to frontend/public/cards/UltraMax/`);
